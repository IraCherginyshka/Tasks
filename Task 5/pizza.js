...
function showList() {
    mainDiv.innerHTML += "<div id='wrapRadio'></div>";
    ...
    mainDiv.innerHTML += "<table><tbody id='div1'></tbody></table>";
...
function showReticle() {
    mainDiv.innerHTML += "<div id='chooseIngredient'></div>";
    ...
    mainDiv.innerHTML += "<div id='wrapReticle'></div>";
    let wrapReticle = document.getElementById("wrapReticle");

    function writeReticle(arr) {
        wrapReticle.innerHTML = "";
        for (let i = 0; i < arr.length; i++) {
            wrapReticle.innerHTML += "<div class='blockPizza'></div>";
            let blockPizza = document.getElementsByClassName('blockPizza');
            blockPizza[i].style.backgroundImage = arr[i].image;
            blockPizza[i].innerText = arr[i].name;

            blockPizza[i].innerHTML += "<p class='ingredientsBlock'></p>";
            let ingredientsBlock = document.getElementsByClassName('ingredientsBlock');
            let str = "";
            for (let j = 0; j < arr[i].composition.length; j++) {
                str += arr[i].composition[j] + ", ";
            }
            ingredientsBlock[i].innerText = "Состав: " + str;

            blockPizza[i].innerHTML += "<p class='caloriesBlock'></p>";
            let caloriesBlock = document.getElementsByClassName('caloriesBlock');
            caloriesBlock[i].innerText = arr[i].totalCalories() + "ккал";

            blockPizza[i].innerHTML += "<p class='priceBlock'></p>";
